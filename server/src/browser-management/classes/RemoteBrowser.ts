...
import { getDecryptedProxyConfig } from '../../services/proxyConfig';
 ...
 */
  private async applyEnhancedFingerprinting(context: BrowserContext): Promise<void> {
     ...
     */
    public initialize = async (userId: string, profile?: BrowserLaunchProfile & {
      userAgent?: string;
      storageStatePath?: string;
      contextProxy?: { server: string; username?: string; password?: string } | null;
      locale?: string;
    }): Promise<void> => {
        const MAX_RETRIES = 3;
        ...
        const initializationPromise = (async () => {
          while (!success && retryCount < MAX_RETRIES) {
            try {
              this.browser = await connectToRemoteBrowser(undefined, profile);
              ...
              if (profile?.contextProxy?.server) {
                proxyOptions = {
                  server: profile.contextProxy.server,
                  username: profile.contextProxy.username,
                  password: profile.contextProxy.password,
                };
              }
              ...
              const contextOptions: any = {
                ...
                isMobile: false,
                hasTouch: false,
                userAgent: profile?.userAgent || this.getUserAgent(),
                locale: profile?.locale || 'en-US',
                storageState: profile?.storageStatePath || undefined,
              };
              ...
              await applyStealthOverrides(this.context, contextOptions.userAgent);
              ...
              // CDP sessions are only available in Chromium, skip for Firefox/Camoufox
              try {

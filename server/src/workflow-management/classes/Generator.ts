...
export class WorkflowGenerator {
  ...
  private async getLastUsedSelectorInfo(page: Page, selector: string) {
    ...
    if (elementHandle) {
      const { tagName, innerText } = await elementHandle.evaluate((el) => {
        ...
        if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
          ...
          text = htmlEl.innerText?.trim() || el.href || '';
        } else {
          text = htmlEl.innerText || '';
        }
        return { tagName: tag, innerText: text };
      });
    ...
    try {
      if (robotId) {
        ...
        if (robot) {
          ...
          logger.log('info', `Robot retrained with id: ${robot._id}`);
        }
      } else {
        ...
        const allUserRobots = await Robot.find({ userId } as any);
        ...
        actionType = 'saved';
        logger.log('info', `Robot saved with id: ${robot._id}`);
      }  
    }
    catch (e: any) {
      if (e.code === 11000) {
  ...
  private IsOverShadowingAction = async (pair: WhereWhatPair, page: Page) => {
    ...
    if (haveSameUrl.length !== 0) {
      for (let i = 0; i < haveSameUrl.length; i++) {
        const p = haveSameUrl[i];
        const selectors = p.where.selectors;
        if (Array.isArray(selectors) && selectors.length > 0) {
          const isOverShadowing = await isRuleOvershadowing(selectors, page);
          if (isOverShadowing && possibleOverShadow[i]) {

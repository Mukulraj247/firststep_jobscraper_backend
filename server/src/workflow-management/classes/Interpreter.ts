 ...
 * maxun-core Interpreter behaves like the extension/Engine 2 and reliably
 * dismisses cookie banners and pauses on CAPTCHA.
 */
function buildEnginePaginationHooks(settings: any) {
  const popups: OverlayDismisserOptions = settings?.popups || {};
  const captcha: CaptchaGateOptions = settings?.captcha || {};
  const autoDismiss = popups.autoDismiss !== false;
  const pauseOnCaptcha = captcha.pauseOnDetect !== false;
  return {
    dismissOverlays: async (page: Page) => {
      if (!autoDismiss) return 0;
      try {
        return await dismissNow(page, popups);
      } catch {
        return 0;
      }
    },
    assertNoCaptcha: async (page: Page) => {
      if (!pauseOnCaptcha) {
        return { present: false } as any;
      }
      return assertNoCaptcha(page, captcha);
    },
...
export class WorkflowInterpreter {
  ...
  public interpretRecordingInEditor = async (
    ...
    const options = {
      ...settings,
      mode: 'editor',
      paginationHooks: buildEnginePaginationHooks(settings as any),
      debugChannel: {
        activeId: (id: any) => {
          this.activeId = id;
    ...
    const status = await interpreter.run(page as any, params);
  ...
  private persistBinaryDataToDatabase = async (binaryItem: { name: string; mimeType: string; data: string }): Promise<void> => {
    ...
    try {
      const run = await Run.findOne({ runId: this.currentRunId });
      if (!run) {
      ...
      run.binaryOutput = updatedBinaryOutput;
      await run.save();
  ...
  public async flushPersistenceBuffer(): Promise<void> {
    ...
    try {
      ...
      if (hasUpdates) {
        run.serializableOutput = currentSerializableOutput;
        await run.save();

...
export async function writeDataToSheet(
  ...
  try {
    ...
    if (!robot) {
      robot = await Robot.findOne({ 'recording_meta.id': robotId });
    ...
    oauth2Client.once('tokens', async (tokens: any) => {
      if (tokens.refresh_token || tokens.access_token) {
        const robotModel = await Robot.findOne({ 'recording_meta.id': robotId });
        if (robotModel) {
          if (tokens.refresh_token) robotModel.google_refresh_token = tokens.refresh_token;
          if (tokens.access_token) robotModel.google_access_token = tokens.access_token;
          await robotModel.save();
        }
      }
    });

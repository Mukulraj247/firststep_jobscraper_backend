...
import { withTimeout, addGeneratedFlags } from "./record/helpers";
...
async function createWorkflowAndStoreMetadata(id: string, userId: string, isSDK: boolean) {
    try {
        ...
        const run = await Run.create({
            ...
            robotId: recording._id ? recording._id.toString() : null,
...
async function executeRun(id: string, userId: string, requestedFormats?: string[]) {
    ...
    try {
        ...
        const extractedRows = await persistExtractedDataForRun(updatedRun, recording);
        await dispatchAutomationWebhook(updatedRun, recording, extractedRows);

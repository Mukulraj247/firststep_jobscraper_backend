...
/** Human-friendly column order for job-style lists; remaining keys sorted A–Z. */
const SCRAPE_LIST_COLUMN_PRIORITY = [
  'jobTitle', 'title', 'name',
  'companyName', 'company', 'employer',
  'location', 'jobLocation',
  'salary', 'compensation',
  'datePosted', 'posted', 'postedAt',
  'jobCategory', 'department',
  'jobUrl', 'url', 'link', 'href',
  'jobDescription', 'description', 'summary',
];

function sortScrapeTableColumnKeys(keys: string[]): string[] {
  const remaining = new Set(keys);
  const ordered: string[] = [];
  for (const k of SCRAPE_LIST_COLUMN_PRIORITY) {
    if (remaining.has(k)) {
      ordered.push(k);
      remaining.delete(k);
    }
      ...
      Object.keys(scrapeListData).forEach(key => {
        ...
        if (Array.isArray(tableData) && tableData.length > 0) {
          ...
          if (filteredData.length > 0) {
            ...
            columnsList.push(sortScrapeTableColumnKeys(Array.from(tableColumns)));
          }
        }
      });
    }
